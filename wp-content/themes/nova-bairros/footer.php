...
<footer class="mt-16 md:mt-28">
    <div class="container">
                ...
                <?php if (!empty($redes)): ?>
                    <div class="flex flex-row gap-3">
                        <?php foreach ($redes as $rede):
                            if (empty($rede['url']) && empty($rede['icone']))
                                continue;
                            ?>
                            <a href="<?= esc_url($rede['url'] ?? '#'); ?>" target="_blank" rel="noopener noreferrer"
                                aria-label="<?= esc_attr($rede['aria_label'] ?? ''); ?>"
                                class="dif-item flex flex-row items-center gap-3">
                                <div
                                    class="icon-social rounded-full w-10 h-10 relative bg-(--amarelo) content-center justify-center items-center flex hover:bg-(--amarelo) transition-colors duration-300">
                                    <?php if (!empty($rede['icone'])): ?>
                                        <img src="<?= esc_url($rede['icone']['url']); ?>"
                                            alt="<?= esc_attr($rede['aria_label'] ?? ''); ?>" class="w-5 h-5">
                                    <?php endif; ?>
                                </div>
                            </a>
                        <?php endforeach; ?>
            ...
            <div class="col-span-12 md:col-span-6 lg:col-span-3 flex flex-col gap-5 wow fadeIn" data-wow-delay="0.2s">
                ...
                <nav class="flex flex-col gap-3">
                    ...
                    <a class="!text-white/80 hover:!text-white transition-colors"
                        href="<?= home_url(); ?>/sobre">Sobre</a>
                    <a class="!text-white/80 hover:!text-white transition-colors"
                        href="<?= home_url(); ?>/empreendimentos">Empreendimentos</a>
                    <a class="!text-white/80 hover:!text-white transition-colors"
                        href="<?= home_url(); ?>/blog">Blog</a>
                    <a class="!text-white/80 hover:!text-white transition-colors"
                        href="<?= home_url(); ?>/contato">Contato</a>
                </nav>
            </div>
                    ...
                    <div class="flex flex-col gap-4">
                        <?php foreach ($contatos_footer as $contato):
                            ...
                            <a href="<?= esc_attr($contato['url'] ?? '#'); ?>" target="_blank"
                                class="dif-item flex flex-row items-center gap-3">
                                <div
                                    class="rounded-full w-10 h-10 shrink-0 relative bg-white/20 items-center justify-center flex">
                                    <?php if (!empty($contato['icone'])): ?>
                                        <img src="<?= esc_url($contato['icone']['url']); ?>" alt=""
                    ...
                    <div class="flex flex-col gap-3">
                        <?php foreach ($links_uteis['links'] as $link):
                            if (empty($link['texto']))
                                continue;
                            ?>
                            <a href="<?= esc_url($link['url'] ?? '#'); ?>" <?= esc_html($link['texto']) == 'Política de Privacidade' || esc_html($link['texto']) == 'Cartilha de Compliance' ? '' : 'target="_blank"' ?>
                                class="cta text-sm! !inline-flex items-center gap-2 transition-colors">
                                <img src="<?= esc_url($link['icone']['url'] ?? ''); ?>" alt="icone-util" class="w-5 h-5">
                                <?= esc_html($link['texto']); ?>
                            </a>
                        <?php endforeach; ?>
                        <a href="#topo" class="cta text-sm! !inline-flex items-center gap-2 transition-colors">
                            <img src="<?= site_url('/wp-content/uploads/2026/07/meteor-icons_arrow-up.svg') ?>"
                                alt="icone-util" class="w-5 h-5">
                            Clique para ir para o topo
                        </a>
                    </div>
                </div>
            <?php endif; ?>
        ...
        <!-- Barra de copyright -->
        <div class="border-t border-white/20 flex justify-between items-center py-6 wow fadeIn bg-[#00663B]"
            data-wow-delay="0.2s">
            <div class="flex flex-col md:flex-row text-center text-sm items-center gap-5 justify-between w-full">
                ...
                <div class="hidden md:block w-[1px] h-4 bg-white/20 mx-3"></div> -->
                <a class="!text-white/60" target="_blank" href="https://alfamaweb.com.br"><img
                        src="<?= IMG_URI ?>alfamaw.svg" alt="AlfamaWeb" loading="lazy"></a>
            </div>
        </div>
    </div>
</footer>
...
<script type='text/javascript'
    src='<?php echo get_template_directory_uri(); ?>/assets/libs/fancybox/fancybox.umd.js'></script>
...
<script type="text/javascript" async
    src="https://d335luupugsy2.cloudfront.net/js/loader-scripts/1a5ab772-c897-40a0-a505-89b20e59081a-loader.js"></script>
