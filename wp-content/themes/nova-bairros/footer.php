            ...
            <?php if (!empty($links_uteis) && !empty($links_uteis['links'])): ?>
                <div class="col-span-12 md:col-span-6 lg:col-span-3 flex flex-col gap-5 wow fadeIn" data-wow-delay="0.6s">
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        ?>
                            <a href="<?= esc_url($link['url'] ?? '#'); ?>" <?= esc_html($link['texto']) == 'Política de Privacidade' || esc_html($link['texto']) == 'Cartilha de Compliance' ? '' : 'target="_blank"' ?> class="cta text-sm! !inline-flex items-center gap-2 hover:!text-white transition-colors">
...
<?php if (!empty($whatsapp_flutuante)): ?>
    <a aria-label="Falar com Corretor"
        ...
        target="_blank" rel="noopener noreferrer"
        class="fixed bottom-6 md:bottom-12 lg:bottom-12 right-4 lg:right-11 w-fit h-fit z-50 <?= is_page('canal-de-denuncia') ? 'hidden' : '' ?>">
